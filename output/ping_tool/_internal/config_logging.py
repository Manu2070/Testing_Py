...
def read_config():
    ...
    print(f"Settings INI Path: {settings_path}")
    logging.info(f"[{ts}] [{settings_path}]")
    print(f"Ports INI Path: {ports_path}")
    logging.info(f"[{ts}] [{ports_path}]")
    
    # Read both config files
    config = configparser.ConfigParser()
    config.read(settings_path)
    if not config.sections():
        print("Settings file not loaded or is empty!")
    ...
    if not config_port.sections():
        print("Ports file not loaded or is empty!")
        logging.info(f"[{ts}] [Ports file not loaded or is empty!]")
    
    return config, config_port

def setup_logging():
    log_dir = os.path.join('Network_stuff', 'log')
